...
// compute_environments has been deprecated in favor of compute_environment_order
func TestAccBatchJobQueue_basicCE(t *testing.T) {
	ctx := acctest.Context(t)
	var jobQueue1 batch.JobQueueDetail
	resourceName := "aws_batch_job_queue.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccJobQueueConfig_stateCE(rName, batch.JQStateEnabled),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckJobQueueExists(ctx, resourceName, &jobQueue1),
					acctest.CheckResourceAttrRegionalARN(resourceName, "arn", "batch", fmt.Sprintf("job-queue/%s", rName)),
...
func testAccJobQueueConfig_stateCE(rName string, state string) string {
	return acctest.ConfigCompose(
		testAccJobQueueConfigBase(rName),
		fmt.Sprintf(`
resource "aws_batch_job_queue" "test" {
